<?php
    class WC_Gateway_Loja5_Woo_Cielo_Webservice_Boleto extends WC_Payment_Gateway {
        ...
        public function init_form_fields() {
			...
			if(version_compare(PHP_VERSION, '5.4.0', '<')) {
				include(CIELO_WEBSERVICE_WOO_PATH.'/include/php53/config_boleto.php' );
			}elseif(version_compare(PHP_VERSION, '5.5.0', '<')) {
				include(CIELO_WEBSERVICE_WOO_PATH.'/include/php54/config_boleto.php' );
			}elseif(version_compare(PHP_VERSION, '5.6.0', '<')){
        ...
        public function process_payment($order_id) {
            ...
            $number = $this->limpar(isset($order->billing_number)?$order->billing_number:'*');
            if ($number == "sn" || $number == "SN")
                $number = "semnumero";
			...
			$dados['Payment'] = array(
				'Type' => 'Boleto',
				'BoletoNumber' => str_pad($order->get_order_number(), 9, "0", STR_PAD_LEFT),
				...
				'Provider' => trim($this->settings['meios']),
			...
			if(($response->status==200 || $response->status==201) && isset($dados_pedido['Payment']['PaymentId'])){
				...
				//salva os dados do boleto 
				$data = array();
				$data['nosso_numero'] = $objResposta['tid'];
				$data['numero_documento'] = $order->get_id();
				$data['data_vencimento'] = date( 'd/m/Y', $validade );
				$data['data_documento'] = date( 'd/m/Y' );
				$data['link_boleto'] = trim($objResposta['bin']);
				$data['linha_digitavel'] = trim($objResposta['linha']);
				update_post_meta( $order->get_id(), 'loja5_woo_cielo_webservice_boleto_dados', $data );
            ...
            if(isset($objResposta['tid']) && $erro==false){
				...
				$wpdb->query("INSERT INTO `wp_cielo_api_loja5` (`id`, `metodo`, `id_pagamento`, `tid`, `pedido`, `bandeira`, `parcela`, `lr`, `lr_log`, `total`, `status`, `bin`, `link`) VALUES (NULL, 'boleto', '".$objResposta['id_pagamento']."', '".$objResposta['tid']."', '".$order->get_id()."', '".$objResposta['bandeira']."', '".$objResposta['parcelas']."', '".$objResposta['lr']."', '".$objResposta['lr_log']."', '".$order->get_total()."', '0', '', '".$objResposta['bin']."');");
				
				//cria uma nota no pedido
				$order->add_order_note("Transa&ccedil;&atilde;o Boleto - ID ".$objResposta['id_pagamento']." via ".strtoupper($objResposta['bandeira'])."");
