...
final class THEMECOMPLETE_EPO_Admin_base {
						...
						if ( isset( $items['tm_item_id'] ) && isset( $items['tm_key'] ) && $items['tm_key'] == $key && $items['tm_item_id'] == $item_id ) {
							...
							if ( isset( $epo['price'] ) ) {
								...
								if ( ! $new_currency ) {
									...
									$epo['price'] = apply_filters( 'wc_epo_remove_current_currency_price', $epo['price'], THEMECOMPLETE_EPO()->get_element_price_type( $saved_epos[ $key ] ), get_option( 'woocommerce_currency' ), $order_currency, $_current_currency_prices, isset( $saved_epos[ $key ]['key'] ) ? $saved_epos[ $key ]['key'] : NULL );
								}
		...
		if ( $has_epo ) {
			...
			if ( $epos && is_array( $epos ) ) {
				...
				foreach ( $epos as $key => $epo ) {
					...
					if ( $epo && is_array( $epo ) ) {
						$type         = THEMECOMPLETE_EPO()->get_element_price_type( $epo );
						$new_currency = FALSE;
	...
	public function register_data_tab( $tabs ) {
		...
		$tabs['tm_extra_product_options'] = array(
			'label'  => esc_html__( 'TM Extra Product Options', 'woocommerce-tm-extra-product-options' ),
