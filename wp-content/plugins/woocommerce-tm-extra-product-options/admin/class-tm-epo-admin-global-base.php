...
final class THEMECOMPLETE_EPO_ADMIN_Global_base {
		...
		if ( ! empty( $post_id ) ) {
			...
			if ( ! empty( $tm_metas ) && is_array( $tm_metas ) && isset( $tm_metas['tm_meta'] ) && is_array( $tm_metas['tm_meta'] ) ) {
				...
				if ( $post && property_exists( $post, 'ID' ) && property_exists( $post, 'post_type' ) ) {
					...
					$message = esc_html__( 'Options saved!', 'woocommerce-tm-extra-product-options' );
					$result  = 1;
	...
	public function tm_variations_check() {
		...
		if ( isset( $_POST['post_id'] ) ) {
			...
			THEMECOMPLETE_EPO_BUILDER()->builder_sub_variations_options( $meta, $post_id );
		}
		die();
	}
	...
	public function admin_menu() {
		$page_hook = add_submenu_page( 'edit.php?post_type=product', esc_html__( 'TM Global Extra Product Options', 'woocommerce-tm-extra-product-options' ), esc_html__( 'TM Global Extra Product Options', 'woocommerce-tm-extra-product-options' ), 'manage_woocommerce', THEMECOMPLETE_EPO_GLOBAL_POST_TYPE_PAGE_HOOK, array( $this, 'admin_screen' ) );
		...
		switch ( $column ) {
			...
			case "applied_on":
				...
				if ( $tm_meta_cat ) {
					if ( ! empty( $tm_meta_id ) && is_array( $tm_meta_id ) ) {
						...
						echo '<span class="tm_color_pomegranate">' . esc_html__( 'Disabled', 'woocommerce-tm-extra-product-options' ) . '</span>';
					}
				} else {
			...
			case 'product_ids' :
				...
				if ( ! empty( $tm_meta ) ) {
					...
					echo '<span class="tm_color_pomegranate">' . esc_html__( 'None', 'woocommerce-tm-extra-product-options' ) . '</span>';
				}
				break;
	...
	public function tm_form_fields_builder_meta_box( $post ) {
        ...
        <div id="tmformfieldsbuilderwrap" class="tm_wrapper">
			...
			if ( $wpml_is_original_product ) {
				echo '<button type="button" id="builder_add_section" class="builder_add_section tc tc-button large"><span class="tc-button-label"><i class="tcfa tcfa-plus-square"></i> ' . esc_html__( "Add section", 'woocommerce-tm-extra-product-options' ) . '</span></button>'
			...
			echo '<button type="button" id="builder_fullsize_close" class="tc tc-button builder_fullsize_close" title="' . esc_attr__( "Close", 'woocommerce-tm-extra-product-options' ) . '"><span class="tc-button-label"><i class="tcfa tcfa-window-minimize"></i></span></button>'
			     . '<button type="button" id="builder_fullsize" class="tc tc-button builder_fullsize" title="' . esc_attr__( "Fullsize", 'woocommerce-tm-extra-product-options' ) . '"><span class="tc-button-label"><i class="tcfa tcfa-window-maximize"></i></span></button>'
			     . '<input id="builder_import_file" name="builder_import_file" type="file" class="builder-import-file" />'
			     . '</div>'
			     . '</div>'
			     . "</div>";

			echo '<div class="builder_layout">';
			...
			if ( $show_buttons ){
				...
				$buttons_html = '<div class="tc-buttons">'
					. '<button type="button" class="tm-animated tc-add-element tc tc-button large" title="' . esc_html__( 'Add element', 'woocommerce-tm-extra-product-options' ) . '"><i class="ddd"></i>' . esc_html__( 'Add element', 'woocommerce-tm-extra-product-options' ) . '</button>'
					...
					. '<button type="button" class="tm-animated tc-add-import-csv tc tc-button large" title="' . esc_html__( 'Import CSV', 'woocommerce-tm-extra-product-options' ) . '"><i class="ddd"></i>' . esc_html__( 'Import CSV', 'woocommerce-tm-extra-product-options' ) . '</button>'
					. '</div>';
			} else {
				$info_html = '<div class="tc-info-text">' 
					. esc_html__( 'There are no options', 'woocommerce-tm-extra-product-options' ) 
					. '</div>';
				if ( $post->post_type === THEMECOMPLETE_EPO_GLOBAL_POST_TYPE ){
					$buttons_html = '<div class="tc-warning-text">' 
						. esc_html__( 'Please save the global form before creating or importing options!', 'woocommerce-tm-extra-product-options' ) 
						. '</div>';
				} else {
					$buttons_html = '<div class="tc-warning-text">' 
						. esc_html__( 'Please save the product before creating or importing options!', 'woocommerce-tm-extra-product-options' ) 
						. '</div>';
				}
			}
			if ( $wpml_is_original_product && THEMECOMPLETE_EPO()->tm_epo_global_hide_product_builder_mode != "yes" ) {
				echo apply_filters( 'wc_epo_admin_tc_welcome', 
					'<div id="tc-welcome" class="tc-welcome">'
					. '<div class="tc-logo"><span class="tm-logo-text"><span class="tm-logo-text-e">E</span><span class="tm-logo-text-po">PO</span></span></div>'
					. $info_html
					. $buttons_html
					. '</div>' );
	...
	public function register_admin_styles( $override = 0 ) {
		...
		wp_enqueue_style( 'themecomplete-font-awesome', $this->plugin_url . '/assets/css/font-awesome' . $ext . '.css', FALSE, '4.7', 'screen' );
	...
	public function js_element_data( $button_class = "" ) {
		...
		$out .= '<div class="tm-box tma-tab-label">'
		        . '<h4 class="tab-header open" data-tm-tag="' . esc_attr( "all" ) . '" data-id="tc-tag' . $tag_counter . '-tab">' . esc_html__( 'All', 'woocommerce-tm-extra-product-options' ) . '</h4>'
		        . '</div>';
	...
	public function save_imported_csv( $post_id ) {
		...
		if ( ! empty( $tm_metas ) && is_array( $tm_metas ) && isset( $tm_metas['tm_meta'] ) && is_array( $tm_metas['tm_meta'] ) ) {
			...
			if ( $post && property_exists( $post, 'ID' ) && property_exists( $post, 'post_type' ) ) {
				...
				THEMECOMPLETE_EPO_BUILDER()->print_saved_elements( $original_post_id, $post_id, $wpml_is_original_product );
			}
		}
