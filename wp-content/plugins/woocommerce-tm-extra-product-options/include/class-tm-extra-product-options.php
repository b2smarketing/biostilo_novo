...
final class THEMECOMPLETE_Extra_Product_Options {
	...
	public function tm_woocommerce_get_price( $price = 0, $product = FALSE ) {
		...
		if ( ! is_admin() && ! $this->wc_vars['is_product'] && $this->tm_epo_use_from_on_price == "no" ) {
			...
			if ( $this->tm_woocommerce_get_price_flag < 2 ) {
				add_filter( 'woocommerce_get_price_html', array( $this, 'tm_get_price_html' ), 11, 2 );
				add_filter( 'woocommerce_get_variation_price_html', array( $this, 'tm_get_price_html' ), 11, 2 );
			}
	...
	public function tm_woocommerce_show_variation_price( $show = TRUE, $product = FALSE, $variation = FALSE ) {
		...
		if ( $product && $variation ) {
			$epos = $this->get_product_tm_epos( themecomplete_get_id( $product ) );
			if ( is_array( $epos ) && ( ! empty( $epos['global'] ) || ! empty( $epos['local'] ) ) ) {
				if ( ! empty( $epos['price'] ) ) {
	...
	public function related_get_price_html2( $price = "", $product = "" ) {
		...
		if ( $product && is_object( $product ) && is_callable( array( $product, "get_price" ) ) ) {
			...
			if ( (float) $product->get_price() > 0 ) {
				...
				$thiscpf = $this->get_product_tm_epos( themecomplete_get_id( $product ) );
	...
	public function is_visible( $element = array(), $section = array(), $sections = array(), $form_prefix = "" ) {
		...
		$this->current_element_to_check = array();

		return $this->is_visible_do( $element, $section, $sections, $form_prefix );
	}
	 ...
	 * @since 1.0
	 */
	private function is_visible_do( $element = array(), $section = array(), $sections = array(), $form_prefix = "" ) {
		...
		if ( isset( $this->visible_elements[ $uniqid ] ) ) {
			return $this->visible_elements[ $uniqid ];
		}
		...
		if ( $is_element ) {
			// Element
			if ( ! $this->is_visible_do( $section, array(), $sections, $form_prefix ) ) {
				$this->visible_elements[ $uniqid ] = FALSE;

				return FALSE;
			}
			if ( ! isset( $element['logic'] ) || empty( $element['logic'] ) ) {
				$this->visible_elements[ $uniqid ] = TRUE;
		...
		if ( $logic ) {
			...
			if ( in_array( $uniqid, $this->current_element_to_check ) ) {
				return TRUE;
			}
			
			$this->current_element_to_check[] = $uniqid;
			...
			$this->visible_elements[ $uniqid ] = $show;
	...
	public function tm_check_field_match( $rule = FALSE, $sections = FALSE, $form_prefix = "" ) {
		...
		if ( isset( $this->visible_elements[ $element_uniqueid ] ) ) {
			if ( ! $this->visible_elements[ $element_uniqueid ] ) {
				return FALSE;
			}
		} else {
			if ( in_array( $element_uniqueid, $this->current_element_to_check ) ) {
			...
			if ( ! $this->is_visible_do( $sections[ $section_id ]['elements'][ $element_id ], $sections[ $section_id ], $sections, $form_prefix ) ) {
				return FALSE;
			}
		}
	...
	public function tm_check_section_match( $element_id, $operator, $rule = FALSE, $sections = FALSE, $form_prefix = "" ) {
		...
		if ( isset( $sections[ $section_id ] ) && isset( $sections[ $section_id ]['elements'] ) ) {
			foreach ( $sections[ $section_id ]['elements'] as $id => $element ) {
				if ( $this->is_visible_do( $element, $sections[ $section_id ], $sections, $form_prefix ) ) {
	 ...
	 * @since 1.0
	 */
	public function get_product_tm_epos( $post_id = 0, $form_prefix = "", $no_cache = FALSE ) {
		...
		$epos                 = $this->generate_global_epos( $tmglobalprices, $post_id, $this->tm_original_builder_elements, $variations_for_conditional_logic, $no_cache );
			...
			foreach ( $tmlocalprices as $price ) {
				...
				if ( $product_epos[ $tmcp_id ]['is_taxonomy'] ) {
					if ( ! ( $attributes[ $product_epos[ $tmcp_id ]['name'] ]['is_variation'] ) ) {
						$all_terms = THEMECOMPLETE_EPO_WPML()->get_terms( NULL, $attributes[ $product_epos[ $tmcp_id ]['name'] ]['name'], 'orderby=name&hide_empty=0' );
						if ( $all_terms ) {
	 ...
	 * @return array
	 */
	public function generate_global_epos( $tmglobalprices, $post_id, $tm_original_builder_elements, $variations_for_conditional_logic = array(), $no_cache = FALSE ) {
		$global_epos         	  = array();
		$product_epos_uniqids 	  = array();
		...
		if ( $tmglobalprices ) {
			...
			foreach ( THEMECOMPLETE_EPO_BUILDER()->_section_elements as $key => $value ) {
					...
					if ( is_array( $builder ) && count( $builder ) > 0 && isset( $builder['element_type'] ) && is_array( $builder['element_type'] ) && count( $builder['element_type'] ) > 0 ) {
						...
						for ( $_s = 0; $_s < count( $_sections ); $_s ++ ) {
							...
							for ( $k0 = $_helper_counter; $k0 < intval( $_helper_counter + intval( $_sections[ $_s ] ) ); $k0 ++ ) {
								...
								if ( isset( THEMECOMPLETE_EPO_BUILDER()->elements_array[ $current_element ] ) ) {
									foreach ( THEMECOMPLETE_EPO_BUILDER()->elements_array[ $current_element ] as $key => $value ) {
										if ( isset( $value['id'] ) && empty( $value['wpmldisable'] ) ) {
									...
									if ( $tm_original_builder_elements[ $current_element ] ) {
										...
										if ( $tm_original_builder_elements[ $current_element ]["type"] == "single" || $tm_original_builder_elements[ $current_element ]["type"] == "multipleallsingle" ) {
											...
											if ( isset( $builder[ 'multiple_' . $current_element . '_options_price' ][ $current_counter ] ) ) {
												...
												$_values  = $this->get_builder_element( 'multiple_' . $current_element . '_options_value', $builder, $current_builder, $current_counter, "", TRUE, $current_element, "wc_epo_multiple_values", $element_uniqueid );
												$_titles  = $this->get_builder_element( 'multiple_' . $current_element . '_options_title', $builder, $current_builder, $current_counter, "", TRUE, $current_element, "wc_epo_multiple_titles", $element_uniqueid );
												...
												$mt_prefix    = THEMECOMPLETE_EPO_HELPER()->get_currency_price_prefix();
												foreach ( $_prices as $_n => $_price ) {
													if ( isset( $_enabled[$_n] ) &&  $_enabled[$_n] === "0" ){
														continue;
													}
													$new_currency = FALSE;
													if ( $mt_prefix !== ''
						...
						if ( isset( $this->tm_builder_elements[ $element['type'] ] ) && $this->tm_builder_elements[ $element['type'] ]["is_post"] == "post" ) {
							...
							if ( $this->tm_builder_elements[ $element['type'] ]["type"] == "multipleall" || $this->tm_builder_elements[ $element['type'] ]["type"] == "multiple" ) {
								...
								foreach ( $element['options'] as $value => $label ) {
									...
									$name_inc = apply_filters( 'wc_epo_name_inc', $name_inc, $base_name_inc, $element, $value );
									...
									$global_epos = apply_filters( 'global_epos_fill_builder_display', $global_epos, $priority, $pid, $_s, $arr_element_counter, $element, $value );
								...
								$name_inc = apply_filters( 'wc_epo_name_inc', $name_inc, $base_name_inc, $element, FALSE );

								$name_inc                                                                                               = 'tmcp_' . $name_inc . ( ( $form_prefix !== "" ) ? "_" . str_replace( "_", "", $form_prefix ) : "" );
								$global_epos[ $priority ][ $pid ]['sections'][ $_s ]['elements'][ $arr_element_counter ]['name_inc']    = $name_inc;
								$global_epos[ $priority ][ $pid ]['sections'][ $_s ]['elements'][ $arr_element_counter ]['is_cart_fee'] = $is_cart_fee;

								$global_epos = apply_filters( 'global_epos_fill_builder_display', $global_epos, $priority, $pid, $_s, $arr_element_counter, $element, FALSE );
