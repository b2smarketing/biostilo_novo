...
class THEMECOMPLETE_EPO_Display {
	...
	public function tm_add_inline_style() {
		...
		if ( ! empty( $this->inline_styles ) ) {
			if ( THEMECOMPLETE_EPO()->is_quick_view() || ( is_ajax() && THEMECOMPLETE_EPO()->is_bto ) ) {
							...
							if ( THEMECOMPLETE_EPO()->tm_builder_elements[ $element['type'] ]["is_post"] == "post" ) {
								...
								if ( THEMECOMPLETE_EPO()->tm_builder_elements[ $element['type'] ]["type"] == "single" || THEMECOMPLETE_EPO()->tm_builder_elements[ $element['type'] ]["type"] == "multipleallsingle" || THEMECOMPLETE_EPO()->tm_builder_elements[ $element['type'] ]["type"] == "multiplesingle" ) {
									...
									$display = $field_obj->display_field( $element, array(
										'id'              => 'tmcp_' . THEMECOMPLETE_EPO()->tm_builder_elements[ $element['type'] ]["post_name_prefix"] . '_' . $tabindex . $form_prefix,
										'name'            => 'tmcp_' . $name_inc,
										'name_inc'        => $name_inc,
										'element_counter' => $element_counter,
										'tabindex'        => $tabindex,
									...
									if ( is_array( $display ) ) {
										...
										$args = apply_filters( 'wc_epo_display_template_args', array_merge( $args, $display ), $element, FALSE );
									...
									foreach ( $element['options'] as $value => $label ) {
										...
										$display = $field_obj->display_field( $element, array(
											'id'              => 'tmcp_' . THEMECOMPLETE_EPO()->tm_builder_elements[ $element['type'] ]["post_name_prefix"] . '_' . $element_counter . "_" . $field_counter . "_" . $tabindex . $form_prefix,
											'name'            => 'tmcp_' . $name_inc,
											'name_inc'        => $name_inc,
											'value'           => $value,
											'label'           => $label,
										...
										if ( is_array( $display ) ) {
											...
											$args = apply_filters( 'wc_epo_display_template_args', array_merge( $args, $display ), $element, $value );
				...
				foreach ( $local_price_array as $field ) {
					if ( isset( $field['name'] ) && isset( $attributes[ $field['name'] ] ) && ! $attributes[ $field['name'] ]['is_variation'] ) {
						...
						if ( $attribute['is_taxonomy'] ) {
							...
							$_current_terms  = THEMECOMPLETE_EPO_WPML()->get_terms( THEMECOMPLETE_EPO_WPML()->get_lang(), $attribute['name'], 'orderby=name&hide_empty=0' );
							...
							$_default_terms  = THEMECOMPLETE_EPO_WPML()->get_terms( THEMECOMPLETE_EPO_WPML()->get_lang(), $attribute['name'], 'orderby=name&hide_empty=0' );
	...
	public function tm_epo_totals( $product_id = 0, $form_prefix = "", $is_from_shortcode = FALSE ) {
		...
		if ( ! property_exists( $woocommerce, 'product_factory' )
		     || $woocommerce->product_factory === NULL
		     || ( $this->tm_options_totals_have_been_displayed && ( ! ( THEMECOMPLETE_EPO()->is_bto || ( ( THEMECOMPLETE_EPO()->is_enabled_shortcodes() && ! $is_from_shortcode ) && ! is_product() ) || ( ( is_shop() || is_product_category() || is_product_tag() ) && THEMECOMPLETE_EPO()->tm_epo_enable_in_shop == "yes" ) ) ) )
		) {
			return;// bad function call
		}

		$this->print_price_fields( $product_id, $form_prefix, $is_from_shortcode );
		if ( ! THEMECOMPLETE_EPO()->is_bto && ! $is_from_shortcode ) {
	...
	private function print_price_fields( $product_id = 0, $form_prefix = "", $is_from_shortcode = FALSE ) {
		...
		$cpf_price_array = THEMECOMPLETE_EPO()->get_product_tm_epos( $product_id );
		if ( ! $cpf_price_array ) {
			return;
		}

		if ( $cpf_price_array && THEMECOMPLETE_EPO()->tm_epo_enable_final_total_box_all == "no" ) {
			$global_price_array = $cpf_price_array['global'];
			$local_price_array  = $cpf_price_array['local'];
			if ( empty( $global_price_array ) && empty( $local_price_array ) ) {
				if ( ! THEMECOMPLETE_EPO()->is_bto ) {
		...
		$regular_price = apply_filters( 'wc_epo_product_price_rules', $regular_price, $product );
		...
		if ( THEMECOMPLETE_EPO()->tm_epo_no_variation_prices_array !== 'yes' ) {
			$all = get_posts( apply_filters( 'woocommerce_variable_children_args', array(
				'post_parent' => themecomplete_get_id( $product ),
			...
			foreach ( $all as $variation ) {
				...
				do_action( 'wc_epo_print_price_fields_in_variation_loop', $variation, $child_id );
