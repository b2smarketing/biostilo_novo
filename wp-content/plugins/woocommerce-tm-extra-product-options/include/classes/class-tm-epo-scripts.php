...
class THEMECOMPLETE_EPO_Scripts {
	...
	public function __construct() {
		...
		add_action( 'wp_head', array( $this, 'frontend_templates' ), 10 );
	...
	public function css_array() {
		...
		if ( THEMECOMPLETE_EPO()->tm_epo_global_js_css_mode == "multiple" || THEMECOMPLETE_EPO()->tm_epo_global_js_css_mode == "dev" ) {
			$css_array = array(
				...
				'themecomplete-font-awesome'    => array(
					'src'     => THEMECOMPLETE_EPO_PLUGIN_URL . '/assets/css/font-awesome' . $ext . '.css',
					'deps'    => FALSE,
					'version' => '4.7',
					'media'   => 'screen',
				),
	...
	public function custom_frontend_scripts() {
		...
		if ( $is_composite || ! is_product() || in_array( "sectionslider", THEMECOMPLETE_EPO()->current_option_features ) ) {
			wp_register_script( 'wc-jquery-ui-touchpunch', WC()->plugin_url() . '/assets/js/jquery-ui-touch-punch/jquery-ui-touch-punch' . $suffix . '.js', array( 'jquery-ui-slider' ), '0.2.3', TRUE );
		...
		if ( THEMECOMPLETE_EPO()->tm_epo_global_js_css_mode == "multiple" || THEMECOMPLETE_EPO()->tm_epo_global_js_css_mode == "dev" ) {
			...
			if ( $is_composite || ! is_product() || ( in_array( "date", THEMECOMPLETE_EPO()->current_option_features ) || in_array( "time", THEMECOMPLETE_EPO()->current_option_features ) ) ) {
			...
			if ( $is_composite || ! is_product() || in_array( "time", THEMECOMPLETE_EPO()->current_option_features ) ) {
