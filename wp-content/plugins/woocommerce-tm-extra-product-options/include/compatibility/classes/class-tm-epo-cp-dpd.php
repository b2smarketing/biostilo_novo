...
final class THEMECOMPLETE_EPO_CP_DPD {
	...
	public function add_compatibility() {
		...
		if ( THEMECOMPLETE_EPO()->tm_epo_dpd_enable == "no" ) {
			...
			add_action( 'rightpress_product_price_late_hook_priority', array( $this, 'rightpress_product_price_late_hook_priority' ), 10, 1 );
		}
	...
	public function epo_can_show_order_price( $can = TURE, $item_meta = array() ) {
		...
		if ( THEMECOMPLETE_EPO()->tm_epo_dpd_show_option_prices_in_order == "no" && empty( $item_meta['tm_has_dpd'] ) && empty( $item_meta['_tm_has_dpd'] ) ) {
			$can = FALSE;
		}
	...
	public function wc_epo_order_item_meta( $item_id, $cart_item_key, $values ) {
		if ( ! empty( $values['tmcartepo'] ) ) {
			if ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '2.7.0', '<' ) ) {
				...
				$item = $item_id;
				$item->add_meta_data( '_tm_has_dpd', 1 );
			}
		}
	}
	...
	public function tm_epo_settings_headers( $headers = array() ) {
		$headers["dpd"] = esc_html__( 'Dynamic Pricing & Discounts', 'woocommerce-tm-extra-product-options' );
