	...
	if ( ! empty( $available_variations ) ) {
		...
		foreach ( $attributes as $name => $options ) {
			...
			if ( sizeof( $attributes ) == $loop ) {
				echo '<a class="reset_variations" href="#reset">' . ( ( ! empty( THEMECOMPLETE_EPO()->tm_epo_reset_variation_text ) ) ? esc_html( THEMECOMPLETE_EPO()->tm_epo_reset_variation_text ) : esc_html__( 'Reset options', 'woocommerce-tm-extra-product-options' ) ) . '</a>';
			}
		}
	}
